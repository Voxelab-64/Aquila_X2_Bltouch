...
inline void Prepare_Item_Bltouch(const uint8_t row){
  if(HMI_flag.language_flag)
    ...
    DWIN_ICON_Show(ICON2,99,LBLX,MBASE(row)+3);
  else
    //DWIN_Frame_AreaCopy(3,0,2,48,20,LBLX, MBASE(row));
  DWIN_ICON_Show(ICON2,98,LBLX,MBASE(row)+3);
    Draw_Menu_Line(row, ICON_CloseMotor);
}
...
//      #else
      ...
      if (HMI_ValueStruct.show_mode == -4) {
        checkkey = Control;
        show_plus_or_minus(font8x16,Black, Background_black, 2, 2, 220, MBASE(5 + MROWS - index_control), TERN(HAS_LEVELING, probe.offset.z * 100, HMI_ValueStruct.offset_value));
      }
      else {
        checkkey = Tune;
...
void update_variable(void) {
  ...
  if (checkkey == TemperatureID) {
    ...
    if (last_probe_zoffset != probe.offset.z) {
        show_plus_or_minus(MENU_FONT,Black, Background_black, 2, 2, 185, 91, probe.offset.z * 100);
...
void Draw_Bltouch_Menu(void)
{
      ...
      if(HMI_flag.language_flag)
      {
          DWIN_ICON_Show(ICON2,107,81,190);                 //title
          DWIN_ICON_Show(ICON2,97,60,260);                 //????
      ...
      { 
          DWIN_ICON_Show(ICON2,106,81,190);                 //title
          DWIN_ICON_Show(ICON2,97,60,260);                //????
      //     DWIN_ICON_Show(ICON1,27,240,80);                 //16?
...
void DWIN_CompletedLeveling(void) {
  ...
  if(HMI_flag.language_flag)
  {
      DWIN_ICON_Show(ICON2,103,80,190);
  }
  else
  {
      DWIN_ICON_Show(ICON2,102,80,190);
  }
...
void BLtouch_detecting(int x,int y)
{
      ...
      if(y==0)
      {
        DWIN_ICON_Show(ICON2,80+4-x,60+40*(3-x),260+40*y);
      }
      else if(y==1)
      {
         DWIN_ICON_Show(ICON2,80+5+x,60+40*(3-x),260+40*y);
      }
      else if(y==2)
      {
         DWIN_ICON_Show(ICON2,88+4-x,60+40*(3-x),260+40*y);
      }
      else if(y==3)
      {
         DWIN_ICON_Show(ICON2,88+5+x,60+40*(3-x),260+40*y);
      }
} 
